...
class LtpParser():
    def __init__(self):
        ...
        #LTP_DIR = "C:/Users/baozi/HYH/OneDrive/R_Python_SQL/#MDD/ltp-3.3.2-win-x64-Release"
        self.segmentor = Segmentor(os.path.join(LTP_DIR, "cws.model"))
        #self.segmentor.load(os.path.join(LTP_DIR, "cws.model")) # pyltp==0.4.0
        ...
        if entity_list:
            for entity in entity_list:
                ...
                if start_index == entity_dict['end_index']:
                    consist = [words[int(float(start_index))] + '/' + postags[int(float(start_index))]]
                else:
                    consist = [words[index] + '/' + postags[index] for index in range(int(float(start_index)), int(float(end_index))+1)]
    ...
    def syntax_parser(self, words, postags):
        ...
        for index in range(len(words)-1):
            print("arcs head", type(arcs))
            arc_index = arcs[index][0] #arc_index = arcs[index].head
            arc_relation = arcs[index][1] #arc_relation = arcs[index].relation
